...
// Tables to track in Hasura after creation
const TABLES_TO_TRACK: Array<{ schema: string; name: string }> = [
...
export async function ensureSchema(): Promise<void> {
  ...
  for (const { schema, name } of TABLES_TO_TRACK) {
    await trackTable(schema, name);
    console.log(`  ✓ Table "${schema}.${name}" is tracked in Hasura`);
  }
}
